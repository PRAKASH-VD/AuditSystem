      ...
      <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[280px_1fr]">
        <aside className="relative overflow-hidden border-r border-black/10 bg-white/70 px-6 py-8 shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
          ...
          <div className="relative z-10">
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center justify-between rounded-2xl px-4 py-3 text-sm font-medium transition ${
                      isActive ? 'bg-[color:var(--ink)] text-white' : 'hover:bg-black/5'
                    }`
                  }
                >
            ...
            <div className="mt-12 rounded-3xl border border-black/10 bg-white p-4">
              <p className="text-xs uppercase tracking-widest text-[color:var(--muted)]">Active Profile</p>
              <p className="mt-2 text-sm font-semibold">{user?.name || 'User'}</p>
              <p className="text-xs text-[color:var(--muted)]">{user?.email || 'email@domain.com'}</p>
              ...
              {user?.role !== 'admin' ? (
                <button
                  ...
                  className="mt-3 w-full rounded-xl border border-dashed border-black/20 px-3 py-2 text-xs font-semibold text-[color:var(--accent)]"
                >
                  Request Admin
                </button>
              ) : null}
        ...
        <main className="px-6 py-8 lg:px-10">
          <header className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-sm uppercase tracking-[0.3em] text-[color:var(--muted)]">Audit Command Center</p>
            ...
            <div className="flex items-center gap-3">
              ...
              <NavLink to="/app/uploads" className="rounded-full bg-[color:var(--accent)] px-4 py-2 text-xs font-semibold text-white shadow-lg shadow-orange-200">
                Upload File
              </NavLink>
            </div>
          </header>
